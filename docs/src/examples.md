...
Build the QTT circulant operator generated by a kernel, apply it as circular convolution, and compare it with zero-padded linear convolution.
...
y_periodic = qtt_cconv(kernel_qtt, signal_qtt)
y_linear = qtt_conv(kernel_qtt, signal_qtt)

periodic_ref = dense_cconv(kernel, signal)
linear_ref = dense_conv(kernel, signal)

println("periodic error: ", norm(qtt_to_vector(y_periodic) - periodic_ref) / norm(periodic_ref))
println("linear error:   ", norm(qtt_to_vector(y_linear) - linear_ref) / norm(linear_ref))
println("circulant QTT rank: ", maximum(C.tto_rks))
```
