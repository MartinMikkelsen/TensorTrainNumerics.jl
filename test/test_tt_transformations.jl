...
@testset "cheb_lobatto_grid" begin
    ...
    @test isapprox(P.grid[1], 0.0; atol = 1.0e-14)
    @test isapprox(P.grid[end], 1.0; atol = 1.0e-14)
    # Check weights at endpoints
    @test isapprox(abs(P.w[1]), 0.5; atol = 1.0e-14)
    @test isapprox(abs(P.w[end]), 0.5; atol = 1.0e-14)
...
@testset "lagrange_eval" begin
    ...
    for α in 0:K
        xα = xs[α + 1]
        # At node, should be 1
        @test isapprox(lagrange_eval(P, α, xα), 1.0; atol = 1.0e-12)
        # At other nodes, should be 0
        for β in 0:K
            if β != α
                xβ = xs[β + 1]
                @test isapprox(lagrange_eval(P, α, xβ), 0.0; atol = 1.0e-12)
            end
        end
    end
    ...
    for x in range(0, stop = 1, length = 10)
        ...
        @test isapprox(s, 1.0; atol = 1.0e-12)
    end
...
@testset "qft_core_entry" begin
    ...
    for α in 0:K, β in 0:K, σ in 0:1, τ in 0:1
        ...
        if τ == 0
            ...
            @test isapprox(real(val), expected; atol = 1.0e-12)
            @test isapprox(imag(val), 0.0; atol = 1.0e-12)
        end
    end
