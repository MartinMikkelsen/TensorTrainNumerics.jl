...
function W(nodes::Vector{Float64}; start::Float64=0.0, stop::Float64=1.0)
    N = length(nodes)-1
    D = N+1

    # compute Clenshaw–Curtis weights on [-1,1]
    w = zeros(Float64, D)
    for j in 0:N
        s = 0.0
        for m in 0:fld(N,2)
            factor = (m==0 || (iseven(N)&&m==N÷2)) ? 1.0 : 2.0
            s += factor/(1-(2m)^2) * cos(2*m*j*pi/N)
        end
        w[j+1] = (2/N)*s
    end
    ...
    # pack into a (1×2×1×D) core by repeating in σ
...
"""
function integrating_qtt(core::Int, N::Int; node_type::String="chebyshev",
    start::Float64=0.0, stop::Float64=1.0
)
    nodes = get_nodes(N, node_type)
    Al = W(nodes; start=start, stop=stop)
    Ac = A_C(nodes)
    Ar = A_R(nodes)
