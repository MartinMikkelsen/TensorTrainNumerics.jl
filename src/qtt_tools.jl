...
qtt_to_function(q::QTTvector) = qtt_to_function(TTvector(q))
...
"""
    circulant_qtto(kernel::QTTvector)

Construct the circulant QTT operator generated by a one-dimensional QTT vector,
preserving QTT metadata.
"""
function circulant_qtto(kernel::QTTvector)
    @assert kernel.n_dims == 1 "circulant_qtto for QTTvector currently expects n_dims == 1"
    return QTToperator(circulant_qtto(TTvector(kernel)), kernel.n_dims, kernel.bits_per_dim, kernel.ordering)
end

"""
    qtt_cconv(kernel::QTTvector, x::QTTvector)

Compute circular convolution between compatible one-dimensional QTT vectors,
preserving QTT metadata.
"""
function qtt_cconv(kernel::QTTvector, x::QTTvector)
    check_compat(kernel, x)
    @assert kernel.n_dims == 1 "qtt_cconv for QTTvector currently expects n_dims == 1"
    return QTTvector(qtt_cconv(TTvector(kernel), TTvector(x)), x.n_dims, x.bits_per_dim, x.ordering)
end

"""
    qtt_conv(kernel::QTTvector, x::QTTvector)

Compute linear convolution between compatible one-dimensional QTT vectors. The
result has one additional bit per dimension from zero-padding.
"""
function qtt_conv(kernel::QTTvector, x::QTTvector)
    check_compat(kernel, x)
    @assert kernel.n_dims == 1 "qtt_conv for QTTvector currently expects n_dims == 1"
    return QTTvector(qtt_conv(TTvector(kernel), TTvector(x)), x.n_dims, x.bits_per_dim + 1, x.ordering)
end
