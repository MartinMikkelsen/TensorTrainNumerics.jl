...
@inline function _subtraction_digit(row_bit::Int, col_bit::Int, borrow_in::Int)
    diff = row_bit - col_bit - borrow_in
    return diff >= 0 ? (diff, 0) : (diff + 2, 1)
end

@inline _borrow_rank_index(rank_index::Int, borrow::Int) = 2 * (rank_index - 1) + borrow + 1

"""
    circulant_qtto(kernel::TTvector)

Construct the QTT operator for the circulant matrix generated by `kernel`.

The dense convention is `C[i, j] = kernel[mod(i - j, 2^d) + 1]` with
zero-based indices inside the modulo. Applying the returned operator therefore
computes periodic convolution with `kernel`.
"""
function circulant_qtto(kernel::TTvector{T, d}) where {T <: Number, d}
    @assert all(==(2), kernel.ttv_dims) "circulant_qtto expects a QTT vector with all physical dimensions equal to 2"
    ...
    if d == 1
        ...
        @inbounds for row_bit in 0:1, col_bit in 0:1
            digit = mod(row_bit - col_bit, 2)
            core[row_bit + 1, col_bit + 1, 1, 1] = kernel.ttv_vec[1][digit + 1, 1, 1]
        end
        return TToperator{T, d}(d, [core], dims, [1, 1], zeros(Int64, d))
    end

    rks = Vector{Int64}(undef, d + 1)
    rks[1] = 1
    rks[end] = 1
    @inbounds for k in 2:d
        rks[k] = 2 * kernel.ttv_rks[k]
    end

    first_core = zeros(T, 2, 2, 1, rks[2])
    @inbounds for row_bit in 0:1, col_bit in 0:1, αr in 1:kernel.ttv_rks[2], borrow_in in 0:1
        digit, _ = _subtraction_digit(row_bit, col_bit, borrow_in)
        right = _borrow_rank_index(αr, borrow_in)
        first_core[row_bit + 1, col_bit + 1, 1, right] += kernel.ttv_vec[1][digit + 1, 1, αr]
    end
    cores[1] = first_core

    @inbounds for k in 2:(d - 1)
        core = zeros(T, 2, 2, rks[k], rks[k + 1])
        for row_bit in 0:1, col_bit in 0:1, αl in 1:kernel.ttv_rks[k], αr in 1:kernel.ttv_rks[k + 1], borrow_in in 0:1
            digit, borrow_out = _subtraction_digit(row_bit, col_bit, borrow_in)
            left = _borrow_rank_index(αl, borrow_out)
            right = _borrow_rank_index(αr, borrow_in)
            core[row_bit + 1, col_bit + 1, left, right] = kernel.ttv_vec[k][digit + 1, αl, αr]
        end
        cores[k] = core
    end

    last_core = zeros(T, 2, 2, rks[d], 1)
    @inbounds for row_bit in 0:1, col_bit in 0:1, αl in 1:kernel.ttv_rks[d]
        digit, borrow_out = _subtraction_digit(row_bit, col_bit, 0)
        left = _borrow_rank_index(αl, borrow_out)
        last_core[row_bit + 1, col_bit + 1, left, 1] = kernel.ttv_vec[d][digit + 1, αl, 1]
    end
    cores[d] = last_core
...
"""
    qtt_cconv(kernel::TTvector, x::TTvector)

Compute circular convolution in QTT form by applying `circulant_qtto(kernel)` to
`x`.
"""
function qtt_cconv(kernel::TTvector{T, d}, x::TTvector{T, d}) where {T <: Number, d}
    @assert kernel.ttv_dims == x.ttv_dims "kernel and x must have matching QTT dimensions"
    @assert all(==(2), kernel.ttv_dims) "qtt_cconv expects QTT vectors with all physical dimensions equal to 2"
    return circulant_qtto(kernel) * x
end

function _qtt_zero_pad_right(x::TTvector{T, d}) where {T <: Number, d}
    @assert all(==(2), x.ttv_dims) "qtt_conv expects QTT vectors with all physical dimensions equal to 2"

    cores = Array{Array{T, 3}, 1}(undef, d + 1)
    selector = zeros(T, 2, 1, 1)
    selector[1, 1, 1] = one(T)
    cores[1] = selector
    @inbounds for k in 1:d
        cores[k + 1] = x.ttv_vec[k]
    end

    dims = ntuple(_ -> 2, d + 1)
    rks = Vector{Int64}(undef, d + 2)
    rks[1] = 1
    @inbounds for k in 1:(d + 1)
        rks[k + 1] = x.ttv_rks[k]
    end
    return TTvector{T, d + 1}(d + 1, cores, dims, rks, zeros(Int64, d + 1))
end

"""
    qtt_conv(kernel::TTvector, x::TTvector)

Compute the linear convolution of two equal-length QTT vectors. The inputs are
zero-padded to length `2^(d + 1)` and multiplied by the corresponding circulant
operator, so the returned vector has one additional QTT bit.
"""
function qtt_conv(kernel::TTvector{T, d}, x::TTvector{T, d}) where {T <: Number, d}
    @assert kernel.ttv_dims == x.ttv_dims "kernel and x must have matching QTT dimensions"
    return qtt_cconv(_qtt_zero_pad_right(kernel), _qtt_zero_pad_right(x))
end
